...
def conj_restructure(g_nodes, g_edges):
    ...
    g_nodes = g_nodes._append(added_v, ignore_index=True)
    g_edges = g_edges._append(added_e, ignore_index=True)
...
def cases_handler(graph):
    ...
    while len(cases) > 0:
        ...
        cases = cases[~(cases['text_vsource'].str.contains('^dalam hal$',case=True) |
                        cases['text_vtarget'].str.contains('^dalam hal$', case=True))]
...
def if_else_construction(graph):
    ...
    except_cond['is_act_type'] = 0
    except_cond['new_rel_name'] = except_cond['rel_name_target']
    ...
    try:
        ...
        filtered_edges = g_edges[g_edges['id'].isin(cond_source.id_sin.array) &
                                g_edges['rel_name'].isin(['advcl', 'parataxis'])]
        filtered_cond_source = cond_source[cond_source['id_sin'].isin(cond_source.id_sin.array) &
                                        cond_source['rel_name_sin'].isin(['advcl', 'parataxis'])]
        g_edges.loc[filtered_edges.index, ['target', 'rel_name']] = filtered_cond_source['new_source']
        g_edges.loc[filtered_edges.index, 'rel_name'] = 'has_condition'
        # EDITED!!
...
def numeral_construction(graph, modality):
    ...
    for num in nums.iterrows():
        ...
        if is_continue:
            ...
            g_nodes = g_nodes._append(new_v, ignore_index=True)
            added_e.append({'sentence_id': sentence_id, 'id': new_eid,
            ...
            if len(verb) > 0:
                ...
                new_v1 = {'sentence_id': sentence_id, 'id': new_vid + 2, 'text': 'memenuhi', 'type': 'verb',
                          'min': -1, 'max': -1, 'is_root_sentence': is_root_sentence, 'state': 1}
                g_nodes = g_nodes._append([new_v, new_v1], ignore_index=True)
                new_eid = len(g_edges)
                source = new_vid + 1
                if noun_in == 'root':
...
def postag_finishing(graph):
    ...
    #                                                                 False]
    new_node['text'] = new_node['rel_name']
    new_node['type'] = 'has_qualifier'
    new_node['min'] = '-1'
    new_node['max'] = '-1'
    new_node['is_root_sentence'] = False
    ...
    #                                                     'has_qualifier_type']
    new_edge['id'] = new_edge['temp']
    new_edge['source'] = new_edge['target']
    new_edge['target'] = new_edge['id_target']
    new_edge['rel_name'] = 'has_qualifier_type'
    ...
    #                                                                                              'temp'] - n_nodes]
    new_node_1['id'] = new_node_1['temp']
    new_node_1['text'] = 'Concept'
    new_node_1['type'] = 'concept'
    new_node_1['min'] = '-1'
    new_node_1['max'] = '-1'
    new_node_1['is_root_sentence'] = False
    new_node_1['state'] = 1
    new_node_1['rank'] = new_node_1['temp'] - n_nodes
    ...
    new_edge_1['id'] = new_edge_1['temp']
    new_edge_1['source'] = new_edge_1['id_source']
    new_edge_1['target'] = new_edge_1['id_target']
    new_edge_1['rel_name'] = 'has_qualifier_type'
    ...
    new_edge_2['id'] = new_edge_2['temp']
    new_edge_2['source'] = new_edge_2['id_source']
    new_edge_2['rel_name'] = 'has_qualifier_value'
    ...
    vlttr['uri'] = [uri.replace('LX', stype) for uri, stype in zip(vlttr['uri'], vlttr['stype'])]
    ...
    lnode['uri'] = [uri.replace('LX', stype) for uri, stype in zip(lnode['uri'], lnode['stype'])]
    ...
    concept_per_sentence['concepts'] = concept_per_sentence['uri'].str.extract(data + r'(?:numeral)?concept_(\d+)', flags=re.IGNORECASE)
    ...
    new_edge1['rel_name'] = schema + 'hasCondition'
    new_edge1['state'] = 1
    new_edge1['is_root_sentence'] = False
    new_edge1['id'] = new_edge1['rank3'].rank()

    # new_edge2[['rel_name', 'state', 'is_root_sentence', 'id']] = [schema + 'hasActType', 1, False,
    #                                                               new_edge2['rank1'].rank()]
    new_edge2['rel_name'] = schema + 'hasActType'
    new_edge2['state'] = 1
    new_edge2['is_root_sentence'] = False
    new_edge2['id'] = new_edge2['rank1'].rank()

    # new_edge3[['rel_name', 'state', 'is_root_sentence', 'id']] = [schema + 'hasSubject', 1, False,
    #                                                               new_edge3['rank3'].rank()]
    new_edge3['rel_name'] = schema + 'hasSubject'
    new_edge3['state'] = 1
    new_edge3['is_root_sentence'] = False
    new_edge3['id'] = new_edge3['rank3'].rank()
...
def postag_handle(graph, modified, deleted):
    ...
    graph['V'] = graph['V'].drop(['level_0', 'index'], axis=1)
    graph['E'] = graph['E'][
    ...
    graph['V']['sentence_id'] = [sentence_id.replace('LX', stype) for sentence_id, stype in zip(graph['V']['sentence_id'], graph['V']['stype'])]
    ...
    graph['E']['sentence_id'] = [sentence_id.replace('LX', stype) for sentence_id, stype in zip(graph['E']['sentence_id'], graph['E']['stype'])]
...
def get_sentence_triple(graph, refer, alias, concept):
    ...
    if not ref_node.empty:
        ...
        ref_node['sentence_id'] = [sentence_id.replace('LX', stype) for sentence_id, stype in zip(ref_node['sentence_id'], ref_node['stype'])]
...
def get_abstract_triple(abstract_df, modifified, deleted, concept, refer):
    ...
    abstract_df['id'] = [id.replace('LX', stype) for id, stype in zip(abstract_df['id'], abstract_df['stype'])]
...
def get_opening_part(considerans, law_bases, dictum, reg_id_funct):
    ...
    lbasis_label.loc[lbasis_label['code']=='UUD', 'year'] = lbasis_label.loc[lbasis_label['code']=='UUD', 'num']
    lbasis_label.loc[lbasis_label['code'] == 'UUD', 'num'] = ''
    lbasis_label['year'] = lbasis_label['year'].replace(to_replace='Nomor', value='Tahun', regex=True)
    # print(lbasis_label)
    ...
    # print(lbasis_label)
    # print(opening_triples)
    opening_triples = opening_triples._append({'source': data + reg_id_funct, 'rel_name': schema + 'hasDictum',
...
def get_reg_triple(title, wd_cache, promulgation=None, enactment=None):
    ...
    # print('get_reg_triple 1')
    if title.keys().__contains__('change'):
        # print('get_reg_triple 2')
        ...
        # print('get_reg_triple 3')
    # print('get_reg_triple 4')
...
def wikidata_linking(source, rels, target, wd_cache, target_type):
    ...
    if wd_cache is not None:
        ...
        targetCache = None
    if targetCache is None or targetCache.size != 0:
        ...
        if r.ok:
            ...
            for data_funct in response:
                ...
                if wd_cache is not None:
                    wd_cache = wd_cache._append([{'object': target, 'uri': target_uri}], ignore_index=True)
                else:
                    wd_cache = pandas.DataFrame([{'object': target, 'uri': target_uri}])
...
file_list = open('core/pdf_list.txt', 'r+')
global reg_types, prefixs
reg_types = pandas.read_csv('core/regulatory_type.csv')
...
prefixs = pandas.read_csv('core/prefixs.csv').fillna('')
resMap = pandas.read_csv('core/turtle_map.csv').fillna('')
state = True
wdCache = pandas.read_csv('core/wikidata_cache.csv').fillna('').drop_duplicates()
file_num = 0
...
if isForAmendment:
    existed_content = pandas.read_csv('core/existedContentAmendment.csv')
else:
    targetDir = 'sentence_triple'
    filterDoc = None
while state:
    file_num += 1
    text = file_list.readline().strip()
    print()
    print(file_num, text)
    if text == '':
        if len(all_node) > 0:
            if not isForEval:
                resMap.loc[resMap['regulatory'].isin(reg_ids), 'ttl_file'] = re.sub(
                    '{targetDir}/'.format(targetDir=targetDir), '', filetarget)
            else:
            ...
            try:
                ...
                print('g_edges.id.array is empty')
                dumps = 0
    ...
    filesource = text
    filetarget = 'new_turtle_files/' + re.search('^new_parsed_files/(.+)\.json$', text).group(1) + '.ttl'
    dirName = re.sub('/[^/]+$', '', filetarget)
    
    if os.path.exists(filetarget):
        print(f'{filetarget} already exist. Skipping...')
        continue
    ...
    try:
        ...
        if not file['title'].keys().__contains__('change'):
            ...
            partof_triple, modifiedContents, deletedContents = get_amend_body_values(file['body'], segment_table,
                                                                                     ...
                                                                                     file['title']['change'],
                                                                                     file['values'])
        # print(file['considerans'])
        # print('====')
        # print(file['law_based'])
        # print('====')
        # print(file['dictum'])
        # print('====')
        # print(reg_id)
        ...
        if len(nodes) == 0 or len(edges) == 0:
            # print('masuk 6')
            print('Nodes or edges is empty')
            with open('core/error.txt', 'a') as f:
                f.write(f'{filesource}\n')

            wdCache.sort_values('object', ascending=True).to_csv('core/wikidata_cache.csv', index=False)
            dumps = 0
            all_node, all_edge, all_refer, all_alias, all_concept, all_abstract, all_potriple = [], [], [], [], [], [], []
            all_modifiedContent, all_deletedContent, titles = [], [], []
            all_optriple, all_regulatory = [], []
    ...
    if resMap is None:
        if not isForEval:
            ...
            resMap = pandas.DataFrame([{'regulatory': reg_id,
                                        'ttl_file': re.sub('{targetDir}/'.format(
                                            targetDir=targetDir), '',
                                            filetarget)}])
        else:
            resMap = pandas.DataFrame([{'regulatory': reg_id,
    ...
    if dumps == 1:
        ...
        numpy.savetxt(filetarget, all_triple.values, fmt='%s', delimiter=' ')
        resMap.sort_values('regulatory', ascending=True).to_csv(
            'core/{targetDir}/turtle_map.csv'.format(targetDir=targetDir),
            index=False)
        wdCache.sort_values('object', ascending=True).to_csv('core/wikidata_cache.csv', index=False)
